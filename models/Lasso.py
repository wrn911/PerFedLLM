...
class Model:
    ...
    def __init__(self, configs):
        ...
        # Lasso参数
        self.alpha = getattr(configs, 'lasso_alpha', 0.01)  # 正则化强度
        self.max_iter = getattr(configs, 'lasso_max_iter', 1000)
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            raise NotImplementedError("Lasso只支持预测任务")
    ...
    def forecast(self, x_enc):
        ...
        for batch_idx in range(batch_size):
            ...
            if self.training or model_key not in self.fitted_models:
                try:
                    # 创建回归数据（使用历史数据作为训练集）
                    # 由于只有当前序列，我们使用滑动窗口创建训练样本
                    if seq_len >= 2 * self.pred_len:
                        # 有足够数据创建训练样本
                        X, y = self._create_sequences(batch_data,
                                                      min(seq_len // 2, 10),
                                                      min(self.pred_len, seq_len // 4))
                        ...
                        if len(X) > 0:
                            ...
                            # 训练Lasso模型
                            lasso_model = Lasso(alpha=self.alpha, max_iter=self.max_iter)
                            lasso_model.fit(X_scaled, y_scaled)

                            # 存储模型和标准化器
                            self.fitted_models[model_key] = lasso_model
                            self.scalers[model_key] = (scaler_X, scaler_y)

                            # 使用最新的seq_len数据进行预测
                            latest_seq = batch_data.flatten().reshape(1, -1)
                            latest_seq_scaled = scaler_X.transform(latest_seq)
                            pred_scaled = lasso_model.predict(latest_seq_scaled)
                            pred = scaler_y.inverse_transform(pred_scaled.reshape(1, -1))

                            predictions[batch_idx] = pred.reshape(self.pred_len, n_features)
                        else:
                            # 备选方案：使用移动平均
                ...
                # 使用已训练的模型进行预测
                try:
                    lasso_model = self.fitted_models[model_key]
    ...
    def fit_and_predict(self, train_data, test_len):
        ...
        try:
            # 创建训练样本
            window_size = min(seq_len // 3, 20)
            pred_size = min(test_len, seq_len // 5)
